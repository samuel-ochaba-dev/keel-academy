...
lint, type-check, tests, and a production build before it can be trusted. CI
mirrors the exact commands a developer runs locally, so "works on my machine"
...
7. `pnpm test` → `turbo run test` (all package test suites).
8. `pnpm build` → `turbo run build` (velite + `next build`), which also runs
...
The `verify` job sets throwaway values for `AUTH_SECRET` / `AUTH_TRUST_HOST` /
`TURSO_DATABASE_URL`, so both `check-types` and `build` exercise the same
environment contract. These are **not** real secrets — CI proves the app
compiles, not that it can reach a live database. Pages that touch the DB are
dynamic (they call `auth()`), so no query runs during the build.

## Roadmap

- **M10:** `pnpm test` is part of the `verify` job. After its first green run,
  protect `main` with a strict required `verify` status check and pull-request
  rule; see `docs/ops/launch-checklist.md`.
...
- **M14:** add an accessibility + Lighthouse budget gate on the reading pages.
