...
## ADR-011: Reference implementations are rendered source, not downloadable artifacts

**Date:** 2026-07-11
**Status:** Accepted
**Context:** M7 unlocks a "reference implementation" for each chapter after a student submits a passing test result. The reference could be (a) a downloadable tarball/zip of the senior's project, (b) a GitHub repo link, or (c) rendered source inside the app. We needed to choose the delivery format.
**Decision:** Render reference source inline in the app (MDX + syntax-highlighted code blocks), exactly like build-along and novel content. No downloads, no repo links.
**Rationale:**

- **Consistency:** Every other content layer (novel, build-along, lexicon, DSA) is authored as MDX and rendered in-app. References follow the same pipeline — same Velite schema, same Shiki highlighting, same `data-layer` attribute pattern.
- **No infra:** A download endpoint needs storage (S3/Turso blobs), signed URLs, expiry, and abuse detection. Rendered source needs zero new infrastructure.
- **Anti-piracy is already lost:** The student has an enrolled account and can copy-paste from the browser regardless. A zip just makes bulk extraction easier; rendering keeps the friction of intentional copy.
- **Diffability:** Rendered source lets us show `diff` blocks, callouts, and inline annotations — a zip can't.
- **Offline isn't a goal yet:** The CLI already scaffolds the student's own project; the reference is a comparison aid, not a dependency.
  **Consequences:**
- Reference content is authored as MDX in `packages/content/content/references/` — one file per chapter, same frontmatter pattern.
- The test suite remains the contract; the reference is explicitly "a second opinion" (stated in the chapter-page footer).
- If we later want downloadable projects, we add a separate route + storage — this decision doesn't lock us out.
- Long reference files may slow first-contentful-paint; we can lazy-load below the fold if it becomes an issue.
